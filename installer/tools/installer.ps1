...
Start-Process -Wait -FilePath "msiexec.exe" -ArgumentList "/a `"$tempMsiPath`" TARGETDIR=`"$installPath`" INSTALL_ROOT=`"$installerName`" /qb /l*v $tempMsiPath\${installerName}_install.log" -Verb RunAs
